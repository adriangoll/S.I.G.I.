...
import type { IDocenteNomina, ICalificacionDocente } from "../types/docente";

export const useExportarDocentePdf = () => {

    const exportarNominaAPdf = (data: IDocenteNomina[], division: string, materia: string) => {
    ...
    const exportarCalificacionesAPdf = (data: ICalificacionDocente[], instancia: string) => {
        if (!data.length) return;
        const pdf = new jsPDF("portrait", "mm", "a4");
        const generationDate = new Date().toLocaleString("es-AR");
    ...
    return { exportarNominaAPdf, exportarCalificacionesAPdf };
};
